...
class Connection:
    ...
    async def connect(
        ...
        """
        # Create audio streamer for handling audio playback
        audio_streamer = AudioStreamer(sample_rate=48000)
            ...
            while True:
                try:
                    async with websockets.connect(socket_url) as socket:
                        ...
                        receive_task = asyncio.create_task(cls._receive_audio_data(socket, audio_streamer))
    ...
    @classmethod
    async def _receive_audio_data(cls, socket, audio_streamer):
        """
        Receive and process audio data from the WebSocket server.

        Args:
            socket (WebSocketClientProtocol): The WebSocket connection.
            audio_streamer (AudioStreamer): The audio streamer instance for handling audio output.
            ...
            async for message in socket:
                try:
                    ...
                    if json_message.get("type") == "audio_output":
                        ...
                        # Add the audio chunk to the audio streamer
                        audio_streamer.add_audio_chunk(audio_data)
                        print(f"Added audio chunk of {len(audio_data)} bytes to playback queue")
